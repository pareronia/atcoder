package com.github.pareronia.atcoder.abc._213.a;
 ...
 * @see <a href="https://atcoder.jp/contests/abc213/tasks/abc213_a">https://atcoder.jp/contests/abc213/tasks/abc213_a</a>
 */
public class Main {
    ...
    private Result<?> handleTestCase(final Scanner sc, final Integer i) {
        ...
        final int len = Math.max(ba.length(), bb.length());
        ba = String.format("%1$" + len + "s", ba).replace(' ', '0');
        bb = String.format("%1$" + len + "s", bb).replace(' ', '0');
        final char[] bc = new char[ba.length()];
        for (int j = 0; j < ba.length(); j++) {
            if (bb.charAt(j) == '0') {
                bc[j] = ba.charAt(j);
            } else {
                bc[j] = ba.charAt(j) == '0' ? '1' : '0';
            }
        }
        return new Result<>(i, List.of(String.valueOf(Integer.parseInt(new String(bc), 2))));
    }
