package com.github.pareronia.atcoder.abc._212.b;
...
/**
 * B - Weak Password
 * @see <a href="https://atcoder.jp/contests/abc212/tasks/abc212_b">https://atcoder.jp/contests/abc212/tasks/abc212_b</a>
 */
public class Main {
    ...
    private Result<?> handleTestCase(final Scanner sc, final Integer i) {
        ...
        for (int j = 0; j < 3; j++) {
            same &= (s.charAt(j + 1) == s.charAt(j));
            final int digit = Character.digit(s.charAt(j), 10);
            final int next = Character.digit(s.charAt(j + 1), 10);
            follows &= ((next == 0 && digit == 9) || (next == digit + 1));
        }
        return new Result<>(i, List.of(follows || same ? "Weak" : "Strong"));
    }
