package com.github.pareronia.atcoder.abc._212.e;
...
/**
 * E - Safety Journey
 * @see <a href="https://atcoder.jp/contests/abc212/tasks/abc212_e">https://atcoder.jp/contests/abc212/tasks/abc212_e</a>
 */
public class Main {
    ...
    private long bfs(final int n, final int start, final int k, final List<Pair<Integer, Integer>> unu) {
        final Deque<Pair<Integer, Integer>> queue = new ArrayDeque<>();
        long cnt = 0;
        queue.add(Pair.of(start, 1));
        while (!queue.isEmpty()) {
            final Pair<Integer,Integer> trip = queue.poll();
            final Integer last = trip.getOne();
            final Integer size = trip.getTwo();
            if (size == k + 1) {
                if (start == last) {
                    cnt = (cnt + 1) % MOD;
                }
            ...
            for (int j = 1; j <= n; j++) {
                ...
                if (unu.contains(Pair.of(j, last))) {
                    continue;
                }
                final Pair<Integer, Integer> newTrip = Pair.of(j, size + 1);
                queue.add(newTrip);
            }
        }
        return cnt;
    ...
    private Result<?> handleTestCase(final Scanner sc, final Integer i) {
        ...
        final List<Pair<Integer, Integer>> unu = new ArrayList<>(2 * m);
        for (int j = 0; j < m; j++) {
        ...
        long ans = 0;
        ans += (bfs(n, 1, k, unu) % MOD);
        final List<Long> tmp = List.of(ans);
        log(() -> tmp);
